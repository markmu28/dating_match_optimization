...
- 名单文件：`嘉宾名单.xlsx`（建议 `嘉宾名单` sheet，含 `嘉宾类型`、`编号`、`姓名`，可选 `英文名`、`别名`、`是否到场`、`是否VIP`）
- `ranking` 偏好表中，主体 `编号` 列与 `对象1ID/对象2ID` 都可直接填：ID、数字、中文名、英文名、别名
- 如有 `嘉宾姓名` 列，会优先按 `嘉宾姓名` 识别主体

## 1.5) GUI 使用（Streamlit）

- 双击 `相亲分组工具.command`（macOS/Linux）或 `相亲分组工具.bat`（Windows）
- GUI 中名单必须上传；偏好可上传 Excel 或在界面手填
- 手填模式下系统会自动生成每位到场嘉宾行，`对象1ID/对象2ID` 通过下拉选择
- 第二轮必须上传 `outputs/安排结果_第一轮.json`

## 2) 现场最常用命令（直接复制）

### 1. 第一轮（VIP 从名单自动读取，推荐）
...
- VIP 默认读取名单 `是否VIP`；`--privileged-guests` 用于临时追加
- 若某个VIP未填写喜欢对象，会自动忽略该VIP并继续求解
